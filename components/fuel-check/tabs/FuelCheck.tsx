...
export default function FuelChecksManagement() {
  ...
  // Optional: Fetch driver names if API is available
  /*
  useEffect(() => {
    const fetchDriverNames = async () => {
      try {
        ...
        const data = await response.json()
        if (data.success) {
          // Assuming response is { success: true, data: [{ id: 6, name: "John Doe" }, ...] }
          const driverMap = new Map(data.data.map((d: { id: number; name: string }) => [d.id, d.name]))
          setDrivers((prevDrivers) =>
            prevDrivers.map((driver) => ({
              id: driver.id,
              name: driverMap.get(driver.id) || driver.id.toString(),
            })),
          )
        }
      } catch (err) {
        console.error("Failed to fetch driver names:", err)
      }
    }
