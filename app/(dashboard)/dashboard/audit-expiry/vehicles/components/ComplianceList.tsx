...
const ComplianceItem = memo(
  ({
    ...
    const isEditingSubtitle =
      editingField?.id === item.id && editingField?.field === "subtitle";
    const isEditingReference =
      editingField?.id === item.id && editingField?.field === "fieldReference";
    const isEditingFieldName =
      editingField?.id === item.id && editingField?.field === "fieldName";
      ...
      <TableRow className="hover:bg-transparent border-b border-gray-100 last:border-0">
          ...
          <div className="space-y-1">
            ...
            {isEditingSubtitle ? (
              <Input
                ...
                value={item.subtitle}
                onChange={(e) =>
                  updateField(item.id, "subtitle", e.target.value)
                }
                onBlur={() => setEditingField(null)}
                onKeyDown={handleKeyDown}
        ...
        <TableCell className="py-3 align-top">
          <Input
            ...
            value={item.days}
            onChange={(e) =>
              updateField(item.id, "days", Number(e.target.value) || 0)
            }
        ...
        <TableCell className="py-3 align-top">
          <div className="flex justify-center">
            <label className="flex items-center cursor-pointer space-x-2 select-none">
              <input
                ...
                onChange={() =>
                  updateField(
                    item.id,
                    "status",
                    item.status === "before" ? "after" : "before",
                  )
                }
                className="hidden"
                disabled={loading || saving}
              ...
              <span
                className={`text-xs font-medium capitalize transition-colors duration-300 w-12 text-left
      ...
      <Table>
        <TableHeader className="bg-gray-50/50">
          <TableRow className="border-b border-gray-100 hover:bg-gray-50/50">
            <TableHead className="w-[40%] text-xs font-medium text-gray-500 uppercase tracking-wide py-3 pl-4">
              Audit Item
            </TableHead>
            {type === "date" && (
              <TableHead className="w-[20%] text-xs font-medium text-gray-500 uppercase tracking-wide py-3">
                Field Name
              </TableHead>
            )}
            <TableHead className="w-[20%] text-xs font-medium text-gray-500 uppercase tracking-wide py-3">
            ...
            <TableHead className="w-[20%] text-center text-xs font-medium text-gray-500 uppercase tracking-wide py-3">
              Days
            </TableHead>
            <TableHead className="w-[20%] text-center text-xs font-medium text-gray-500 uppercase tracking-wide py-3">
              Trigger
              ...
              <TableRow className="bg-blue-50/30 hover:bg-blue-50/30 border-0">
                ...
                <TableCell className="py-4 align-top">
                  <ReferenceSelect
                    ...
                    onChange={(val) =>
                      setNewItem({ ...newItem, fieldReference: val })
                    }
                    attributes={memoizedAttributes}
                      ...
                      <input
                        ...
                        onChange={() =>
                          setNewItem({
                            ...newItem,
                            status:
                              newItem.status === "before" ? "after" : "before",
                          })
                        }
                        className="hidden"
                      />
