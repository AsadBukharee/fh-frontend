...
export default function VehicleDetailPage() {
  ...
  const handleDateSave = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/vehicles/${id}/`, {
        ...
        body: JSON.stringify({ [editDateField]: tempDate }),
      });
  ...
  const handlePmiUpdateSuccess = () => {
    // Refresh vehicle data after PMI update
    const fetchData = async () => {
      try {
        const vehicleRes = await fetch(`${API_URL}/api/vehicles/${id}/`, {
          headers: { 
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <InspectionDialog
        ...
        username={cookies.get("username") || "User"} // You might need to get this from your auth context
        onUpdateSuccess={handlePmiUpdateSuccess}
      />
