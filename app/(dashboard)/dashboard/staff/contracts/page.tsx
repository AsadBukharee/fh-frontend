...
const ShiftManagement = () => {
  ...
  // Quick assign template to contract
  const handleQuickAssign = async (templateId: number, contractId: number | null) => {
    setAssigningTemplate(templateId)
    try {
      const template = shiftTemplates.find((t) => t.id === templateId)
      if (!template) throw new Error("Template not found")

      const endpoint = `/api/staff/shifts/${templateId}/`
      const payload = {
        name: template.name,
  ...
  const ShiftCard = ({
    shift,
    isTemplate = false,
  }: { shift: Shift | ShiftTemplate; isTemplate?: boolean; contractId?: number | null }) => {
          ...
          {isEditingThis && editedTemplate ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                <Button
                  onClick={() =>
                    isTemplate ? handleSaveTemplate(shift as ShiftTemplate) : handleSaveShift(shift as Shift)
                  }
                  disabled={saving}
            ...
            <div className="space-y-4">
              ...
              {/* Quick Assignment Section for Templates */}
              {isTemplate && (
  ...
  const totalShifts = contracts.reduce((acc, contract) => acc + contract.shifts.length, 0)
  const totalTemplates = shiftTemplates.length
  const averageRate =
                  ...
                  <AccordionContent className="px-6 pb-6">
                    <div className="grid gap-4">
                      {contract.shifts.map((shift: Shift) => (
                        <ShiftCard key={shift.id} shift={shift} contractId={contract.id} />
                      ))}
